...
def run(cmd):
	# Run a command with all output suppressed
...
def sign_file(priv, infile, sigout):
	# Sign a file, suppressing all output
	return subprocess.run(["openssl", "pkeyutl", "-sign", "-inkey", priv, "-in", infile, "-out", sigout], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def verify_file(pub, infile, sigfile):
	# Verify a signature, suppressing all output
	return subprocess.run(["openssl", "pkeyutl", "-verify", "-pubin", "-inkey", pub, "-in", infile, "-sigfile", sigfile], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def pubout_from_priv(priv, pub):
	# Try generic pkey pubout, suppressing all output
